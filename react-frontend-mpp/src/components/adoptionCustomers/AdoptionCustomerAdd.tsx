          ...
          <CardContent>
            ...
            <form onSubmit={addPet}>
              <TextField
                ...
                fullWidth
                sx={{ mb: 2 }}
                onChange={(event) => setAdoptionsCustomer({ ...adoptionCustomer, adoptionContract: event.target.value })}
              />
              <TextField
                id="feedback"
                label="Feedback"
                ...
                sx={{ mb: 2 }}
                onChange={(event) =>
                  setAdoptionsCustomer({ ...adoptionCustomer, customerFeedback: event.target.value })
                }
              />
              <Autocomplete
                id="adoption"
                ...
                onChange={(event, value) => {
                  if (value) {
                      setAdoptionsCustomer({ ...adoptionCustomer, idAdoptionAdoptionCustomer: value.id});
                  }
                }}
              />
            <div>
...
<Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} label="Customer first name" variant="outlined" />
                )}
                filterOptions={(options, state) =>
                  options.filter((option) =>
                    option.firstName.toLocaleLowerCase().includes(state.inputValue.toLocaleLowerCase())
                  )
                }
                onInputChange={handleCustomersInputChange}
                onChange={(event, value) => {
                  if (value) {
                      setAdoptionsCustomer({ ...adoptionCustomer, idCustomerAdoptionCustomer: value.id});
                  }
                }}
              />
